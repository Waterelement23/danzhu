"""Make offline muted rolling auditions from the user-provided hard-floor recording.

Usage: python scripts/process-muted-rolling.py /path/to/marble_tile_drop_and_rolling.wav
...
OUT = Path(__file__).resolve().parents[1] / 'assets/audio/marble-earth-from-tile'
OUT.mkdir(parents=True, exist_ok=True)
SOURCE_HASH = hashlib.sha256(SOURCE.read_bytes()).hexdigest()
FILTERS = {
    'reference': 'highpass=f=80',
    'mild': 'highpass=f=80,equalizer=f=1800:t=q:w=0.7:g=-3,lowpass=f=2800:p=2',
    'muted': 'highpass=f=100,equalizer=f=2200:t=q:w=0.7:g=-5,lowpass=f=1400:p=2',
}
signals = {}
...
manifest = {
    'status': 'rejected by user: still sounds like tile/wood friction; retained for comparison',
    ...
    'processing': 'EQ and low-pass filtering, RMS comparison gain, 12 ms/40 ms edge fades; '
...
write('earth-roll-comparison', np.concatenate(audition), loop=False,
      sequence=['reference', 'mild', 'muted'], silenceBetweenSeconds=.8)
