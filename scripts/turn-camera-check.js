async (page) => {
  ...
  try{
    ...
    await first.locator('#view-toggle').click();await first.waitForTimeout(700);
    const box=await first.locator('#aim-pad').boundingBox(),x=box.x+box.width/2,y=box.y+box.height*.35;
    await first.mouse.move(x,y);await first.mouse.down();await first.mouse.move(x,y+50);
    ...
    await first.keyboard.press('Escape');await first.mouse.up();
    ...
    await first.mouse.move(x,y);await first.mouse.down();await first.mouse.move(x,y+14);
