async (page) => {
  ...
  for(const viewport of [{width:390,height:844},{width:844,height:390}]) {
    ...
    try {
      ...
      check(await p.locator('#drag-origin').count()===0,'No artificial origin marble');
      const ball=await p.evaluate(()=>{const s=window.cameraTest,r=s.container.getBoundingClientRect();return {...s.dragMarble,x:s.dragMarble.x+r.x,y:s.dragMarble.y+r.y};});
      const meter=await p.locator('#drag-feedback').boundingBox();
      check(meter.x+meter.width < ball.x-ball.radius || meter.x>ball.x+ball.radius,'Meter avoids marble');
      check(await p.locator('#drag-feedback').evaluate(e=>getComputedStyle(e).backgroundColor)==='rgba(0, 0, 0, 0)','No meter background');
      const bar=await p.locator('.drag-power').boundingBox();
      check(bar.height>bar.width*10,'Vertical meter');
      await p.screenshot({path:`/tmp/danzhu-meter-${viewport.width}.png`});
      await touch('touchMove',[[ball.x,ball.y+12]]);
      check((await state()).power===0,'Real marble cancels an off-ball gesture');
      ...
      await touch('touchMove',[[ball.x,ball.y+22]]);
      check((await state()).power===0,'Hysteresis preserves cancellation');
      await p.screenshot({path:`/tmp/danzhu-cancel-${viewport.width}.png`});
      await touch('touchMove',[[ball.x,ball.y+32]]);
      check((await state()).power>0,'Pull away restores charge');
      await touch('touchMove',[[ball.x,ball.y]]);await touch('touchEnd',[]);
      check((await state()).shots===0,'Release on marble cancels');
      check(await p.locator('#drag-feedback').isHidden(),'Feedback cleared');
      await touch('touchStart',[[x,y]]);await touch('touchMove',[[x,y+45]]);
      await p.evaluate(({x,y})=>{const s=window.cameraTest;s.renderer.domElement.dispatchEvent(new PointerEvent('pointerup',{pointerId:s.pointerId,pointerType:'touch',clientX:x,clientY:y,bubbles:true}));},ball);
      await touch('touchEnd',[]);
      check((await state()).shots===0,'Release coordinates checked without final move');
      await touch('touchStart',[[ball.x,ball.y]]);await touch('touchMove',[[ball.x,ball.y+45]]);
      await touch('touchMove',[[ball.x,ball.y]]);await touch('touchEnd',[]);
      check((await state()).shots===0,'On-ball start returns to marble to cancel');
      await touch('touchStart',[[x,y]]);await touch('touchMove',[[x,y+45]]);
      ...
      await p.waitForTimeout(100);
      const onBall=await p.locator('.active-ball-ring').evaluate(e=>{const r=e.ownerSVGElement.getBoundingClientRect();return {x:r.x+Number(e.getAttribute('cx')),y:r.y+Number(e.getAttribute('cy'))};});
      await touch('touchStart',[[onBall.x,onBall.y]]);await touch('touchMove',[[onBall.x,onBall.y+14]]);await touch('touchEnd',[]);
      ...
      const target=await p.evaluate(()=>{const s=window.cameraTest,r=s.container.getBoundingClientRect();return {x:s.dragMarble.x+r.x,y:s.dragMarble.y+r.y};});
      await touch('touchMove',[[target.x,target.y]]);
      check((await state()).power===0,'Normal turn cancels on its real marble');
      await touch('touchMove',[[target.x,target.y+32]]);
      await touch('touchEnd',[]);
